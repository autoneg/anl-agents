...
class Agent(embodied.Agent):
  ...
  def train(self, carry, data, env_step=None, total_env_steps=None):
    seed = data.pop('seed')
    data = self._add_run_context(data, env_step, total_env_steps)
  ...
  def report(self, carry, data, env_step=None, total_env_steps=None):
    seed = data.pop('seed')
    data = self._add_run_context(data, env_step, total_env_steps)
    assert sorted(data.keys()) == sorted(self.spaces.keys()), (
        sorted(data.keys()), sorted(self.spaces.keys()))
  ...
  def _add_run_context(self, data, env_step, total_env_steps):
    values = {
        'env_step': env_step,
        'total_env_steps': total_env_steps,
    }
    add = {k: v for k, v in values.items() if k in self.spaces}
    if not add:
      return data
    example = next(iter(data.values()))
    batch_shape = example.shape[:2]
    arrays = {}
    for key, value in add.items():
      if value is None:
        value = 0
      space = self.spaces[key]
      arrays[key] = np.full(batch_shape + space.shape, value, space.dtype)
    return {**data, **internal.device_put(arrays, self.train_sharded)}
