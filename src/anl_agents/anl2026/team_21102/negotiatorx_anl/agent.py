...
class MimicCritic:
    """Small acceptability critic using observed opponent style, not opponent names."""
    ...
    def __init__(
        ...
        rejection_penalty: float = 0.020,
        utility_floor: float = 0.24,
    ) -> None:
        self.mode = mode
        ...
        self.uncertainty_penalty = max(0.0, uncertainty_penalty)
        self.rejection_penalty = max(0.0, rejection_penalty)
    ...
    def bonus(
        ...
        if not self._active(opponent, relative_time, phase):
            return 0.0

        acceptance_fit = opponent.constraint_acceptance_estimate(item.outcome)
        recent_fit = opponent.recent_estimate(item.outcome)
        anchor_similarity = opponent._positive_anchor_similarity(item.outcome)
        rejection_similarity = opponent._rejection_similarity(item.outcome)
        repetition = self._recent_repetition(opponent)
        uncertainty = opponent.outcome_uncertainty(item.outcome)
        ...
        acceptability = (
            ...
            + 0.10 * recent_similarity
            + 0.06 * repetition
        )
        time_pressure = self._time_pressure(relative_time)
        utility_guard = self._utility_guard(item.utility)
        raw_bonus = self.weight * confidence * time_pressure * acceptability
        raw_bonus *= 0.65 + 0.35 * utility_guard
        raw_bonus -= self.uncertainty_penalty * uncertainty
        raw_bonus -= self.rejection_penalty * rejection_similarity

        max_bonus = self.weight
        max_penalty = min(0.025, 0.60 * self.weight + self.rejection_penalty)
        return max(-max_penalty, min(max_bonus, raw_bonus))
    ...
    def trace(
        ...
        active = self._active(opponent, relative_time, phase)
        return {
            "mode": self.mode,
            "active": active,
            "bonus": self.bonus(
            ...
            "uncertainty": opponent.outcome_uncertainty(item.outcome) if active else 0.0,
            "rejection_similarity": opponent._rejection_similarity(item.outcome)
            if active
            else 0.0,
        }
    ...
    def _active(
        ...
        if not self.enabled or phase not in {"bridge", "close"}:
            return False
        if self.mode == "tail" and relative_time < max(self.min_time, 0.74):
            return False
        if relative_time < self.min_time:
            return False
        if len(opponent.history) < self.min_history:
            return False
        return opponent.confidence >= self.min_confidence

    def _time_pressure(self, relative_time: float) -> float:
        span = max(0.001, 1.0 - self.min_time)
        progress = max(0.0, min(1.0, (relative_time - self.min_time) / span))
        return 0.55 + 0.45 * progress

    def _utility_guard(self, utility: float) -> float:
        span = max(0.05, 0.58 - self.utility_floor)
        return max(0.0, min(1.0, (utility - self.utility_floor) / span))
...
class BiddingPolicy:
    def __init__(
        ...
        mimic_critic_mode: str = "off",
        mimic_critic_min_time: float = 0.65,
        mimic_critic_min_history: int = 5,
        mimic_critic_min_confidence: float = 0.45,
        mimic_critic_weight: float = 0.035,
        mimic_critic_uncertainty_penalty: float = 0.010,
        mimic_critic_rejection_penalty: float = 0.020,
        mimic_critic_utility_floor: float = 0.24,
        near_top_diversity_mode: str = "off",
        near_top_diversity_start: float = 0.03,
        near_top_diversity_end: float = 0.74,
        near_top_diversity_max_loss: float = 0.05,
        near_top_diversity_min_candidates: int = 3,
        near_top_diversity_min_opponent: float = 0.22,
        near_top_diversity_weight: float = 0.045,
        astra_lite_mode: str = "off",
        astra_lite_start: float = 0.18,
        astra_lite_end: float = 0.94,
        astra_lite_lambda_base: float = 0.55,
        astra_lite_max_loss: float = 0.10,
        astra_lite_cap_step: float = 0.035,
        astra_lite_cap_slack: float = 0.015,
        astra_lite_min_candidates: int = 2,
        astra_lite_top_k: int = 10,
        astra_lite_acceptance_weight: float = 0.24,
        astra_lite_strategy_weight: float = 0.08,
        astra_lite_repeat_penalty: float = 0.08,
        astra_lite_rejection_penalty: float = 0.20,
    ) -> None:
        ...
        self.mimic_critic = MimicCritic(
            mode=mimic_critic_mode,
            min_time=mimic_critic_min_time,
            min_history=mimic_critic_min_history,
            min_confidence=mimic_critic_min_confidence,
            weight=mimic_critic_weight,
            uncertainty_penalty=mimic_critic_uncertainty_penalty,
            rejection_penalty=mimic_critic_rejection_penalty,
            utility_floor=mimic_critic_utility_floor,
        )
        self.near_top_diversity_mode = near_top_diversity_mode
        self.near_top_diversity_start = near_top_diversity_start
        self.near_top_diversity_end = near_top_diversity_end
        self.near_top_diversity_max_loss = max(0.0, near_top_diversity_max_loss)
        self.near_top_diversity_min_candidates = max(
            1, near_top_diversity_min_candidates
        )
        self.near_top_diversity_min_opponent = max(
            0.0, min(1.0, near_top_diversity_min_opponent)
        )
        self.near_top_diversity_weight = max(0.0, near_top_diversity_weight)
        self.astra_lite_mode = astra_lite_mode
        self.astra_lite_start = astra_lite_start
        self.astra_lite_end = astra_lite_end
        self.astra_lite_lambda_base = max(0.05, min(0.95, astra_lite_lambda_base))
        self.astra_lite_max_loss = max(0.0, astra_lite_max_loss)
        self.astra_lite_cap_step = max(0.001, astra_lite_cap_step)
        self.astra_lite_cap_slack = max(0.0, astra_lite_cap_slack)
        self.astra_lite_min_candidates = max(1, astra_lite_min_candidates)
        self.astra_lite_top_k = max(1, astra_lite_top_k)
        self.astra_lite_acceptance_weight = max(0.0, astra_lite_acceptance_weight)
        self.astra_lite_strategy_weight = max(0.0, astra_lite_strategy_weight)
        self.astra_lite_repeat_penalty = max(0.0, astra_lite_repeat_penalty)
        self.astra_lite_rejection_penalty = max(0.0, astra_lite_rejection_penalty)
    ...
    def choose(
        ...
        astra_lite = False
        if not (rescue or allocation_rescue or constraint_probe or numeric_rescue):
            astra_candidates = self._astra_lite_candidates(
                candidates,
                opponent,
                behavior,
                relative_time,
            ...
            if astra_candidates:
                candidates = astra_candidates
                astra_lite = True
        near_top_diversity = False
        if not (
            astra_lite
            or rescue
            ...
            diverse_candidates = self._near_top_diversity_candidates(
                candidates, opponent, behavior, relative_time, phase
            )
            if diverse_candidates:
                candidates = diverse_candidates
                near_top_diversity = True
    ...
    def score(
        ...
        policy_profile: str = "default",
        near_top_diversity: bool = False,
        astra_lite: bool = False,
    ) -> tuple[float, ...]:
        ...
        mimic_bonus = self.mimic_critic.bonus(
            item,
            opponent,
            relative_time,
            phase,
        ...
        diversity_bonus = 0.0
        if near_top_diversity:
            diversity_bonus = self._near_top_diversity_bonus(
                item,
                opponent_utility,
                relative_time,
                phase,
    ...
    def _astra_lite_candidates(
        ...
        if not self._astra_lite_active(relative_time, phase):
            return []
        if len(candidates) < self.astra_lite_min_candidates:
            return []

        stance = self._astra_lite_stance(opponent)
        base_lambda = self._astra_lite_lambda(stance, behavior, relative_time)
        lambdas = [
            max(0.05, min(0.95, base_lambda + offset))
            for offset in (-0.20, -0.10, 0.0, 0.10, 0.20)
        ]
        min_utility = max(floor, target - self.astra_lite_max_loss)
        caps = [
            max(min_utility, target - self.astra_lite_cap_step * index)
            for index in range(4)
        ]

        selected: dict[Outcome, OutcomeStats] = {}
        for lambda_value in lambdas:
            for cap in caps:
                eligible = [
                    item
                    for item in candidates
                    if item.utility >= min_utility
                    and item.utility <= cap + self.astra_lite_cap_slack
                ]
                if not eligible:
                    eligible = [
                        item for item in candidates if item.utility >= min_utility
                    ]
                if not eligible:
                    continue
                chosen = max(
                    eligible,
                    key=lambda item: self._astra_lite_generation_score(
                        item,
                        opponent,
                        behavior,
                        relative_time,
                ...
                selected[chosen.outcome] = chosen

        if not selected:
            return []
        ranked = sorted(
            selected.values(),
            key=lambda item: self._astra_lite_score(
                item,
                opponent,
                behavior,
                relative_time,
                phase,
                self._opponent_utility(
                    opponent, item.outcome, behavior, relative_time
                ),
                item.utility
                * max(
                ...
                self._recent_similarity(item.outcome, opponent),
            ),
            reverse=True,
        )
        return ranked[: self.astra_lite_top_k]

    def _astra_lite_active(self, relative_time: float, phase: str) -> bool:
        if self.astra_lite_mode == "off":
            return False
        if relative_time < self.astra_lite_start or relative_time > self.astra_lite_end:
            return False
        if self.astra_lite_mode == "tail":
            return phase in {"bridge", "close"}
        return phase in {"explore", "bridge", "close"}

    def _astra_lite_generation_score(
        self,
        item: OutcomeStats,
        opponent: OpponentModel,
        behavior: str,
        ...
        cap_excess = max(0.0, item.utility - cap)
        exact_rejected = 1.0 if item.outcome in opponent.rejected_self_offers[-8:] else 0.0
        return (
            item.utility
            + (1.0 - lambda_value) * opponent_utility
            - 0.45 * cap_excess
            - self.astra_lite_repeat_penalty * self.sent_offers[item.outcome]
            - self.astra_lite_rejection_penalty
            * (opponent._rejection_similarity(item.outcome) + exact_rejected)
        )

    def _astra_lite_score(
        self,
        item: OutcomeStats,
        opponent: OpponentModel,
        behavior: str,
        ...
        exact_rejected = 1.0 if item.outcome in opponent.rejected_self_offers[-8:] else 0.0
        stance = self._astra_lite_stance(opponent)
        fairness = self._astra_lite_fairness(item.utility, opponent_utility)
        strategy_bonus = self._astra_lite_strategy_bonus(
            item,
            opponent,
            stance,
            fairness,
        ...
        utility_weight = 1.16
        acceptance_weight = self.astra_lite_acceptance_weight
        opponent_weight = 0.20
        if self.astra_lite_mode == "tail" and relative_time >= 0.82:
            utility_weight = 1.00
            acceptance_weight += 0.16
            opponent_weight += 0.08
        elif stance == "greedy" or behavior == "hardliner":
            utility_weight += 0.08
            opponent_weight = max(0.10, opponent_weight - 0.06)
        elif stance == "generous":
            acceptance_weight += 0.06
            opponent_weight += 0.05

        score = (
            utility_weight * item.utility
            + opponent_weight * opponent_utility
            + acceptance_weight * acceptance_fit
            + 0.16 * nash_proxy
            + 0.08 * similarity
            + strategy_bonus
            - self.astra_lite_repeat_penalty * self.sent_offers[item.outcome]
            - self.astra_lite_rejection_penalty
            * (rejection_similarity + exact_rejected)
        )
        return (
            score,
            item.utility,
    ...
    def _astra_lite_stance(self, opponent: OpponentModel) -> str:
        utilities = opponent.self_utility_history
        if len(utilities) < 3:
            return "neutral"
        recent = utilities[-3:]
        previous = utilities[-6:-3] if len(utilities) >= 6 else utilities[:-3]
        recent_mean = sum(recent) / len(recent)
        previous_mean = (
            sum(previous) / len(previous) if previous else recent_mean
        )
        delta = recent_mean - previous_mean
        if recent_mean >= 0.70 or delta >= 0.05:
            return "generous"
        if recent_mean <= 0.34 or delta <= -0.04:
            return "greedy"
        return "neutral"

    def _astra_lite_lambda(
        self, stance: str, behavior: str, relative_time: float
    ) -> float:
        value = self.astra_lite_lambda_base
        if stance == "greedy" or behavior == "hardliner":
            value += 0.08
        elif stance == "generous" or behavior == "conceder":
            value -= 0.08
    ...
    @staticmethod
    def _astra_lite_fairness(self_utility: float, opponent_utility: float) -> str:
        diff = self_utility - opponent_utility
        if diff > 0.42:
            return "opponent_low"
        if diff < -0.28:
            return "self_low"
    ...
    def _astra_lite_strategy_bonus(
        self,
        item: OutcomeStats,
        opponent: OpponentModel,
        stance: str,
        ...
        bonus = 0.08 * nash_proxy
        if stance == "generous":
            bonus += 0.05 * opponent_utility + 0.04 * similarity
        elif stance == "greedy" or behavior == "hardliner":
            bonus += 0.06 * item.utility - 0.03 * opponent_utility
        elif behavior == "reciprocal":
            bonus += 0.04 * similarity + 0.03 * opponent_utility

        if fairness == "self_low":
            bonus -= 0.08
        elif fairness == "opponent_low" and phase in {"bridge", "close"}:
            bonus += 0.03 * opponent_utility

        if self.logrolling_mode != "off":
            bonus += self._logrolling_bonus(
                item, opponent, phase, opponent.confidence
            )
        return self.astra_lite_strategy_weight * bonus

    def _near_top_diversity_candidates(
        self,
        candidates: list[OutcomeStats],
        opponent: OpponentModel,
        behavior: str,
        ...
        if not self._near_top_diversity_active(relative_time, phase):
            return []
        if len(candidates) < self.near_top_diversity_min_candidates:
            return []

        best_utility = max(item.utility for item in candidates)
        utility_floor = max(0.0, best_utility - self.near_top_diversity_max_loss)
        near_top = [item for item in candidates if item.utility >= utility_floor]
        if len(near_top) < self.near_top_diversity_min_candidates:
            return []

        min_opponent = self.near_top_diversity_min_opponent
        if opponent.confidence < 0.25:
            min_opponent *= 0.50
        compatible = [
            item
            for item in near_top
            if self._opponent_utility(
                opponent, item.outcome, behavior, relative_time
            )
            >= min_opponent
        ]
        if len(compatible) >= self.near_top_diversity_min_candidates:
            return compatible
        return near_top

    def _near_top_diversity_active(
        self, relative_time: float, phase: str
    ) -> bool:
        if self.near_top_diversity_mode == "off":
            return False
        if phase not in {"anchor", "explore", "bridge"}:
            return False
        if relative_time < self.near_top_diversity_start:
            return False
        if relative_time > self.near_top_diversity_end:
            return False
        return True

    def _near_top_diversity_bonus(
        self,
        item: OutcomeStats,
        opponent_utility: float,
        relative_time: float,
        ...
        sent_diversity = self._sent_offer_diversity(item.outcome)
        opponent_fit = min(
            1.0,
            opponent_utility / max(0.001, self.near_top_diversity_min_opponent),
        )
        pressure = 1.0
        if phase == "anchor":
            pressure = 0.65
            ...
            pressure = 0.80
        time_shape = 1.0 - 0.25 * max(
            0.0,
            min(
                1.0,
                (relative_time - self.near_top_diversity_start)
                / max(
                    0.001,
                    self.near_top_diversity_end
                    - self.near_top_diversity_start,
                ),
            ),
        )
        return (
            self.near_top_diversity_weight
            * pressure
            * time_shape
            * (0.70 * sent_diversity + 0.30 * opponent_fit)
        )

    def _sent_offer_diversity(self, outcome: Outcome) -> float:
        if not self.sent_offers:
            return 0.50
        distances = []
        for previous in self.sent_offers:
            width = max(len(outcome), len(previous), 1)
            shared = min(len(outcome), len(previous))
            mismatches = sum(
                1 for index in range(shared) if outcome[index] != previous[index]
            )
            mismatches += abs(len(outcome) - len(previous))
            distances.append(mismatches / width)
...
class NegotiatorXAnl(SAOCallNegotiator):
    """HAN-derived behavioral negotiator adapted for ANL 2026."""

    _MAX_CATALOG_SIZE = 20000
    _FAST_EXPORT_CATALOG_SIZE = 6000
    _FAST_EXPORT_MAPPING_SIZE = 6000
    ...
    def __init__(
        ...
        policy_switch_guard_mode: str = "off",
        policy_switch_guard_best_seen: float = 0.62,
        policy_switch_guard_min_repetition: float = 0.70,
        policy_switch_guard_max_slope: float = 0.04,
        policy_switch_guard_max_offer_utility: float = 0.42,
        counter_opportunity_mode: str = "off",
        counter_opportunity_min_time: float = 0.88,
        counter_opportunity_min_gain: float = 0.14,
        counter_opportunity_offer_margin: float = 0.08,
        counter_opportunity_min_default_utility: float = 0.48,
        **kwargs: Any,
    ) -> None:
        for legacy_key in (
            "model",
        ...
        policy_switch_guard_mode = os.getenv(
            "NEGOTIATORX_POLICY_SWITCH_GUARD_MODE",
            policy_switch_guard_mode,
        )
        policy_switch_guard_best_seen = self._env_float(
            "NEGOTIATORX_POLICY_SWITCH_GUARD_BEST_SEEN",
            policy_switch_guard_best_seen,
        )
        policy_switch_guard_min_repetition = self._env_float(
            "NEGOTIATORX_POLICY_SWITCH_GUARD_MIN_REPETITION",
            policy_switch_guard_min_repetition,
        )
        policy_switch_guard_max_slope = self._env_float(
            "NEGOTIATORX_POLICY_SWITCH_GUARD_MAX_SLOPE",
            policy_switch_guard_max_slope,
        )
        policy_switch_guard_max_offer_utility = self._env_float(
            "NEGOTIATORX_POLICY_SWITCH_GUARD_MAX_OFFER_UTILITY",
            policy_switch_guard_max_offer_utility,
        )
        counter_opportunity_mode = os.getenv(
            "NEGOTIATORX_COUNTER_OPPORTUNITY_MODE",
            counter_opportunity_mode,
        )
        counter_opportunity_min_time = self._env_float(
            "NEGOTIATORX_COUNTER_OPPORTUNITY_MIN_TIME",
            counter_opportunity_min_time,
        )
        counter_opportunity_min_gain = self._env_float(
            "NEGOTIATORX_COUNTER_OPPORTUNITY_MIN_GAIN",
            counter_opportunity_min_gain,
        )
        counter_opportunity_offer_margin = self._env_float(
            "NEGOTIATORX_COUNTER_OPPORTUNITY_OFFER_MARGIN",
            counter_opportunity_offer_margin,
        )
        counter_opportunity_min_default_utility = self._env_float(
            "NEGOTIATORX_COUNTER_OPPORTUNITY_MIN_DEFAULT_UTILITY",
            counter_opportunity_min_default_utility,
        )
        ...
        self._policy_switch_guard_mode = policy_switch_guard_mode
        self._policy_switch_guard_best_seen = policy_switch_guard_best_seen
        self._policy_switch_guard_min_repetition = policy_switch_guard_min_repetition
        self._policy_switch_guard_max_slope = policy_switch_guard_max_slope
        self._policy_switch_guard_max_offer_utility = (
            policy_switch_guard_max_offer_utility
        )
        self._counter_opportunity_mode = counter_opportunity_mode
        self._counter_opportunity_min_time = counter_opportunity_min_time
        self._counter_opportunity_min_gain = counter_opportunity_min_gain
        self._counter_opportunity_offer_margin = counter_opportunity_offer_margin
        self._counter_opportunity_min_default_utility = (
            counter_opportunity_min_default_utility
        )
        ...
        self._last_export_observations = -1
    ...
    def _env_bidding_kwargs(cls) -> dict[str, Any]:
        string_keys = {
            ...
            "NEGOTIATORX_MIMIC_CRITIC_MODE": "mimic_critic_mode",
            "NEGOTIATORX_NEAR_TOP_DIVERSITY_MODE": "near_top_diversity_mode",
            "NEGOTIATORX_ASTRA_LITE_MODE": "astra_lite_mode",
        }
        float_keys = {
            ...
            "NEGOTIATORX_MIMIC_CRITIC_MIN_TIME": "mimic_critic_min_time",
            "NEGOTIATORX_MIMIC_CRITIC_MIN_CONFIDENCE": (
                "mimic_critic_min_confidence"
            ),
            "NEGOTIATORX_MIMIC_CRITIC_WEIGHT": "mimic_critic_weight",
            "NEGOTIATORX_MIMIC_CRITIC_UNCERTAINTY_PENALTY": (
                "mimic_critic_uncertainty_penalty"
            ),
            "NEGOTIATORX_MIMIC_CRITIC_REJECTION_PENALTY": (
                "mimic_critic_rejection_penalty"
            ),
            "NEGOTIATORX_MIMIC_CRITIC_UTILITY_FLOOR": (
                "mimic_critic_utility_floor"
            ),
            "NEGOTIATORX_NEAR_TOP_DIVERSITY_START": (
                "near_top_diversity_start"
            ),
            "NEGOTIATORX_NEAR_TOP_DIVERSITY_END": "near_top_diversity_end",
            "NEGOTIATORX_NEAR_TOP_DIVERSITY_MAX_LOSS": (
                "near_top_diversity_max_loss"
            ),
            "NEGOTIATORX_NEAR_TOP_DIVERSITY_MIN_OPPONENT": (
                "near_top_diversity_min_opponent"
            ),
            "NEGOTIATORX_NEAR_TOP_DIVERSITY_WEIGHT": (
                "near_top_diversity_weight"
            ),
            "NEGOTIATORX_ASTRA_LITE_START": "astra_lite_start",
            "NEGOTIATORX_ASTRA_LITE_END": "astra_lite_end",
            "NEGOTIATORX_ASTRA_LITE_LAMBDA_BASE": "astra_lite_lambda_base",
            "NEGOTIATORX_ASTRA_LITE_MAX_LOSS": "astra_lite_max_loss",
            "NEGOTIATORX_ASTRA_LITE_CAP_STEP": "astra_lite_cap_step",
            "NEGOTIATORX_ASTRA_LITE_CAP_SLACK": "astra_lite_cap_slack",
            "NEGOTIATORX_ASTRA_LITE_ACCEPTANCE_WEIGHT": (
                "astra_lite_acceptance_weight"
            ),
            "NEGOTIATORX_ASTRA_LITE_STRATEGY_WEIGHT": (
                "astra_lite_strategy_weight"
            ),
            "NEGOTIATORX_ASTRA_LITE_REPEAT_PENALTY": (
                "astra_lite_repeat_penalty"
            ),
            "NEGOTIATORX_ASTRA_LITE_REJECTION_PENALTY": (
                "astra_lite_rejection_penalty"
            ),
        }
        int_keys = {
            "NEGOTIATORX_ANNEAL_TOP_K": "anneal_top_k",
            "NEGOTIATORX_SHORT_HORIZON": "short_horizon",
            "NEGOTIATORX_MIMIC_CRITIC_MIN_HISTORY": "mimic_critic_min_history",
            "NEGOTIATORX_NEAR_TOP_DIVERSITY_MIN_CANDIDATES": (
                "near_top_diversity_min_candidates"
            ),
            "NEGOTIATORX_ASTRA_LITE_MIN_CANDIDATES": (
                "astra_lite_min_candidates"
            ),
            "NEGOTIATORX_ASTRA_LITE_TOP_K": "astra_lite_top_k",
        }
    ...
    def __call__(self, state: SAOState, dest: str | None = None) -> SAOResponse:
        try:
            response = self._respond_han_core(state, dest)
            force_export = response.response == ResponseType.ACCEPT_OFFER
            self._safe_update_exported_model(force=force_export)
            return SAOResponse(response.response, response.outcome)
        except Exception:
            self._safe_update_exported_model()
            fallback = self._safe_fallback_offer()
            if fallback is None:
                return SAOResponse(ResponseType.END_NEGOTIATION, None)
            return SAOResponse(ResponseType.REJECT_OFFER, fallback)

    def _safe_fallback_offer(self) -> Outcome | None:
        if self._catalog:
            return self._catalog[0].outcome
        return None

    def _safe_update_exported_model(self, force: bool = False) -> None:
        try:
            model = self._build_capped_opponent_ufun(force=force)
            if model is not None:
                self.private_info["opponent_ufun"] = model
        except Exception:
            return

    def _respond_han_core(self, state: SAOState, dest: str | None = None) -> SAOResponse:
        ...
        (
            policy_profile,
            next_offer,
            counter_opportunity_trace,
        ) = self._counter_opportunity_adjustment(
            offer_stats,
            behavior,
            relative_time,
            reserved_value,
        ...
        bidding_trace["counter_opportunity_guard"] = counter_opportunity_trace
    ...
    def _ensure_ready(self) -> None:
        ...
        outcomes = list(
            self.nmi.outcome_space.enumerate_or_sample(
                max_cardinality=self._MAX_CATALOG_SIZE
            )
        )
    ...
    def opponent_ufun(self) -> MappingUtilityFunction | None:
        return self._build_capped_opponent_ufun(force=True)

    def _build_capped_opponent_ufun(
        self, force: bool = False
    ) -> MappingUtilityFunction | None:
        if self._catalog is None or self._opponent is None:
            return None
        observation_count = len(self._opponent.history)
        large_catalog = len(self._catalog) > self._FAST_EXPORT_CATALOG_SIZE
        refresh_gap = 60 if large_catalog else (30 if len(self._catalog) > 2000 else 10)
        if (
            not force
            and self._last_export_observations >= 0
            and observation_count - self._last_export_observations < refresh_gap
        ):
            existing = self.private_info.get("opponent_ufun")
            return existing if isinstance(existing, MappingUtilityFunction) else None

        export_catalog = self._catalog
        if large_catalog:
            selected: dict[Outcome, OutcomeStats] = {
                item.outcome: item
                for item in self._catalog[: self._FAST_EXPORT_MAPPING_SIZE]
            }
            for offer in self._opponent.history[-64:]:
                item = self._catalog_by_outcome.get(offer)
                if item is not None:
                    selected[item.outcome] = item
            export_catalog = list(selected.values())

        kwargs = self._effective_export_kwargs()
        if observation_count:
            mapping = {
                item.outcome: self._opponent.export_estimate(item.outcome, **kwargs)
                for item in export_catalog
            }
        else:
            mapping = {item.outcome: 1.0 - item.utility for item in export_catalog}
        self._last_export_observations = observation_count
    ...
    def _policy_profile(
        ...
        if mode == "agreement_rescue":
            ...
            if not hardish and relative_time < self._policy_switch_time + 0.08:
                return "default"
            if not self._policy_switch_guard_allows(offer_stats, repetition, slope):
                return "default"
            return mode
    ...
    def _counter_opportunity_adjustment(
        self,
        offer_stats: OutcomeStats | None,
        behavior: str,
        relative_time: float,
        ...
        prediction_review: dict[str, Any] | None,
        policy_profile: str,
        rescue_offer: OutcomeStats,
    ) -> tuple[str, OutcomeStats, dict[str, Any]]:
        trace = self._counter_opportunity_decision(
            offer_stats=offer_stats,
            rescue_offer=rescue_offer,
            default_offer=None,
            relative_time=relative_time,
            policy_profile=policy_profile,
        )
        if trace["reason"] not in {"needs_default_counter", "default_counter_better"}:
            return policy_profile, rescue_offer, trace

        assert self._catalog is not None
        assert self._opponent is not None
        default_offer = self._bidding.choose(
            self._catalog,
            self._opponent,
            behavior,
            relative_time,
        ...
        if trace["activated"]:
            return "default", default_offer, trace
        return policy_profile, rescue_offer, trace

    def _counter_opportunity_decision(
        self,
        offer_stats: OutcomeStats | None,
        rescue_offer: OutcomeStats,
        default_offer: OutcomeStats | None,
        relative_time: float,
        policy_profile: str,
    ) -> dict[str, Any]:
        result: dict[str, Any] = {
            "mode": self._counter_opportunity_mode,
            "activated": False,
            "reason": "off",
            "policy_profile": policy_profile,
            "rescue_utility": round(rescue_offer.utility, 6),
            "default_utility": None,
            "incoming_utility": (
                round(offer_stats.utility, 6) if offer_stats is not None else None
            ),
            "default_gain": None,
            "default_offer_margin": None,
            "min_time": round(self._counter_opportunity_min_time, 6),
            "min_gain": round(self._counter_opportunity_min_gain, 6),
            "required_offer_margin": round(
                self._counter_opportunity_offer_margin, 6
            ),
            "min_default_utility": round(
                self._counter_opportunity_min_default_utility, 6
            ),
        }
        if self._counter_opportunity_mode == "off":
            return result
        if self._counter_opportunity_mode != "default_counter":
            result["reason"] = "unknown_mode"
            return result
        if policy_profile != "agreement_rescue":
            result["reason"] = "not_rescue_profile"
            return result
        if offer_stats is None:
            result["reason"] = "missing_incoming_offer"
            return result
        if relative_time < self._counter_opportunity_min_time:
            result["reason"] = "too_early"
            return result
        if default_offer is None:
            result["reason"] = "needs_default_counter"
            return result

        default_gain = default_offer.utility - rescue_offer.utility
        offer_margin = default_offer.utility - offer_stats.utility
        result.update(
            {
                "default_utility": round(default_offer.utility, 6),
                "default_gain": round(default_gain, 6),
                "default_offer_margin": round(offer_margin, 6),
            }
        )
        if default_offer.utility < self._counter_opportunity_min_default_utility:
            result["reason"] = "default_below_floor"
            return result
        if default_gain < self._counter_opportunity_min_gain:
            result["reason"] = "gain_too_small"
            return result
        if offer_margin < self._counter_opportunity_offer_margin:
            result["reason"] = "incoming_too_close_to_default"
            return result

        result["activated"] = True
        result["reason"] = "default_counter_better"
        return result

    def _policy_switch_guard_allows(
        self,
        offer_stats: OutcomeStats,
        repetition: float,
        slope: float,
        ...
        return bool(
            self._policy_switch_guard_decision(offer_stats, repetition, slope)[
                "allowed"
            ]
        )

    def _policy_switch_guard_decision(
        self,
        offer_stats: OutcomeStats | None,
        repetition: float | None = None,
        slope: float | None = None,
    ) -> dict[str, Any]:
        mode = self._policy_switch_guard_mode
        result: dict[str, Any] = {
            "mode": mode,
            "allowed": True,
            "reason": "off",
            ...
            "repetition": None,
            "slope": None,
            "current_offer_utility": None,
        }
        if mode == "off":
            return result
        if offer_stats is None or self._opponent is None:
            result["allowed"] = False
            result["reason"] = "missing_offer_or_model"
            return result
        assert self._opponent is not None
        ...
        result.update(
            {
                ...
                "slope": slope,
                "current_offer_utility": offer_stats.utility,
            }
        )
        if mode in {"tail_risk", "balanced_tail"}:
            checks = (
                ("low_best_seen", best_seen <= self._policy_switch_guard_best_seen),
                (
                    "high_repetition",
                    repetition >= self._policy_switch_guard_min_repetition,
                ),
                ("low_slope", slope <= self._policy_switch_guard_max_slope),
                (
                    "low_current_offer",
                    offer_stats.utility <= self._policy_switch_guard_max_offer_utility,
                ),
            )
        elif mode in {"tail_risk_strict", "tail_risk_recovery_aware"}:
            if (
                mode == "tail_risk_recovery_aware"
                and offer_stats.utility >= 0.40
                and repetition < 0.60
                and slope > 0.05
            ):
                result["allowed"] = False
                result["reason"] = "recovering_offer_block"
                return result
            checks = (
                ("low_best_seen", best_seen <= self._policy_switch_guard_best_seen),
                (
                ...
                (
                    "low_slope_and_weak_offer",
                    slope <= self._policy_switch_guard_max_slope
                    and offer_stats.utility <= 0.48,
                ),
            )
        else:
            result["allowed"] = False
            result["reason"] = "unknown_guard_mode"
            return result

        for reason, allowed in checks:
            if allowed:
                result["allowed"] = True
    ...
    def _bidding_trace(
        ...
        return {
            ...
            "policy_switch_guard": {
                ...
                for key, value in self._policy_switch_guard_decision(
                    offer_stats
                ).items()
            },
            ...
            "mimic_critic_mode": self._bidding.mimic_critic.mode,
            "mimic_critic_enabled": self._bidding.mimic_critic.enabled,
    ...
    def _candidate_trace(
        ...
        if self._opponent is not None:
            ...
            mimic_trace = self._bidding.mimic_critic.trace(
                item,
                self._opponent,
                relative_time,
                phase,
                result["estimated_opponent_utility"],
                result["similarity"],
            )
            result["mimic_critic"] = {
                key: round(value, 6) if isinstance(value, float) else value
                for key, value in mimic_trace.items()
            }
        return result
